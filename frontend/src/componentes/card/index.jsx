...
export default function CardInformativo() {
  ...
  const carregarItensDaRefeicao = async (index, idRefeicao) => {
    try {
      ...
      const response = await fetch(`http://localhost:3000/refeicao/${idUsuario}/${idRefeicao}`);
      const dados = await response.json();
  ...
  return (
    <section className="w-full px-4 pb-8 pt-8 sm:px-6 lg:px-8 lg:pt-10">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-4">
        <h1 className="text-2xl font-bold text-[#e5e7eb] sm:text-3xl">Refeições</h1>
      </div>

      {nomeRefeicoes.map((refeicao, index) => {
        const isOpen = openIndex === index;
        return (
          <div key={refeicao.name} className="mx-auto mt-4 flex w-full max-w-6xl flex-col gap-3 rounded-2xl border border-[#333332] bg-[#1a1f27] px-4 py-5">
            <div className="flex justify-between items-center">
              <h3
                ...
                className="cursor-pointer text-[#e5e7eb] text-lg font-semibold"
              >
                {refeicao.name}
              </h3>
              <button onClick={() => { setMealIndex(index); setIsModalOpen(true); }} className="px-3 py-2 bg-[#232933] text-[#e5e7eb] rounded-lg text-sm">+ Alimento</button>
            </div>

            {isOpen && (
              <div className="mt-4 border-t border-[#333332] pt-4">
                {(itensPorRefeicao[index] || []).map((alimento, i) => (
                  <div key={i} className="flex justify-between p-3 border-b border-[#333332] text-[#e5e7eb]">
                    <p>{alimento.nome_alimento}</p> {/* <-- AQUI ESTÁ O NOME QUE O SQL RETORNA */}
                    <p>{alimento.quantidade}x</p>
                  </div>
                ))}
              </div>
            )}
